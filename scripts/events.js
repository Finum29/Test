...
async function openEventModal(eventId) {
  try {
    ...
    const isCheckInOpen = minutesDiff <= 10 && minutesDiff > -60; // Open from 10 mins before until 1 hour after start
    const isRegistrationOpen = minutesDiff >= -5; // Open until 5 mins after start
    ...
    if (event.status === 'live') {
      modalButtons.innerHTML = `
        ...
        ${userRegistered ? `<button class="btn-cancel" onclick="abandonMatch('${event.id}')">Abandon Match</button>` : ''}
    ...
    // Allow registration even if within 10 mins window, as long as it hasn't started (or just started)
...
async function abandonMatch(eventId) {
  if (!confirm('Are you sure you want to abandon this match? This action cannot be undone.')) {
    return;
  }

  try {
    const response = await fetch(`/events/${eventId}/abandon`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' }
    });
    ...
    if (data.ok) {
      alert('You have abandoned the match');
      closeEventModal();
      loadEvents();
    } else {
      alert(data.message);
