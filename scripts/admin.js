...
async function disqualifyUser(userId, username, eventId) {
  try {
    ...
    if (data.ok) {
      alert(`${username} has been disqualified from the event`);
      loadPlayers();
      loadEvents();
    }
  } catch (error) {
...
function drop(ev) {
  ...
  if (dropTarget && draggedElement !== dropTarget) {
      ...
      // In a real app, you would save the new order to the server here
  }
}
...
async function createEvent(e) {
  ...
  const iconUrl = document.getElementById('eventIconUrl').value;
...
async function setStreamUrl(eventId, currentUrl) {
  ...
  if (streamUrl === null) return; // Cancelled
    ...
    if (event.bracket) {
        bracketHtml = `
            <div class="bracket-admin-view" style="margin-top: 20px; overflow-x: auto;">
                <h4 style="color: var(--accent-red); margin-bottom: 10px;">Bracket Management</h4>
    ...
    return `
      <div class="admin-card">
        ...
        <div class="card-body">
          ...
          ${event.abandoned && event.abandoned.length > 0 ? `
            <div style="margin-top: 15px;">
              <strong>Abandoned (${event.abandoned.length}):</strong>
              <ul style="margin-top: 8px; color: var(--gray);">
                ${event.abandoned.map(a => `<li>${a.username}</li>`).join('')}
              </ul>
            </div>
          ` : ''}
...
async function sendLobbyLink(eventId, participantId, participantName) {
    ...
    // In a real app, this would send a DM or notification.
    // For now, we'll use the tournament chat to send a private message if possible, 
    // or just a general notification. Since we don't have DMs, we'll simulate it via notification.
